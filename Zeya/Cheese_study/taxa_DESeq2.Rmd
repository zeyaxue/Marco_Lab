...
```{r}
qPCR.Tur <- read.csv(file.path(path, "mapping/Good_data_Turicibacter_qPCR-1-6-19_NoLOG.csv"))
qPCR.Tur <- qPCR.Tur[,c(-3,-4)] #remove old log value
# subset by PMA treated and untreated
qPCR.TurY <- qPCR.Tur[qPCR.Tur$PMA == "Y",]
qPCR.TurN <- qPCR.Tur[qPCR.Tur$PMA == "N",] 
# melt dataframe to be suited for ploting
qPCR.TurYm <- melt(qPCR.TurY, id.vars = c("Week","CheeseOutcome","SampleType","No_LOG2"),
                   variable.name = "measure",
                   value.name = "value")
qPCR.TurNm <- melt(qPCR.TurY, id.vars = c("Week","CheeseOutcome","SampleType","No_LOG2"),
                   variable.name = "measure",
                   value.name = "value")
...
TuriPlot <- function(df, path.out, w, h){
  pdf(file.path("~/Google Drive File Stream/My Drive/UC_Davis/Marco_lab/milk_microbiota/Hilmar_weekly_samples/",path.out), width = w, height = h)
  print(ggplot(df, aes(x=SampleType, y=No_LOG2, fill=CheeseOutcome))+ 
          geom_boxplot() +
          scale_fill_manual(values=c("no_slits"="#a6cee3", "slits"="#1f78b4")) +
          theme_bw(base_size = 15)+
          ylab("Cells/ml")+
...
ChouANOVA <- function(x){
  fit <- aov(No_LOG2 ~ CheeseOutcome, data = x)
  summary(fit)
}
