...
Bar(ps.ChedY.noL, var1 = "SampleType", 
    lvl1 = c("Cheese_0D","Cheese_120D","Cheese_30D","Cheese_90D"),
    taxa = comm16, mycol = col16,
    path.out = file.path(path, "Mar_2019/ChedY_noL_sampletype.pdf"),7,6)
Bar(ps.ChedN.noL, var1 = "SampleType", 
    lvl1 = c("Cheese_0D","Cheese_120D","Cheese_30D","Cheese_90D"),
    taxa = comm16, mycol = col16,
    path.out = file.path(path, "Mar_2019/ChedN_noL_sampletype.pdf"),7,6)
```
...
## Stacked bar plot
```{r}
ChouBar <- function(ps, taxa, mycol, samType, w, h, path.out){
  TaxTab <- tax_table(ps) %>% as.data.frame()
  taxa_names(ps) <- TaxTab$REC
  allTaxa <- taxa_names(ps)
  ps.notaxa <- prune_taxa(allTaxa[!(allTaxa %in% taxa)], ps)
