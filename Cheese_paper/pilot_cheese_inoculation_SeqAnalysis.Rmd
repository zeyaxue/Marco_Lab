...
TaxTab <- separate(data = TaxTab, col = "Taxon", into = tax_col, sep = "; ", 
                   remove = TRUE, fill = "right")
# format taxon columns to remove preceding taxon letter (" p__, o__"")
# 4th letter start to keep
TaxTab$Kingdom <- substring(TaxTab$Kingdom, 4) %>% factor()
TaxTab$Phylum <- substring(TaxTab$Phylum, 4) %>% factor()
TaxTab$Class <- substring(TaxTab$Class, 4) %>% factor()
TaxTab$Order <- substring(TaxTab$Order, 4) %>% factor()
TaxTab$Family <- substring(TaxTab$Family, 4) %>% factor()
TaxTab$Genus <- substring(TaxTab$Genus, 4) %>% factor()
TaxTab$Species <- substring(TaxTab$Species, 4) %>% factor()
# replace NA with "None" (because some tax level are blank and some are NA)
for (i in 2:(dim(TaxTab)[2])-1) {
  # Get levels and add "None"
  levels <- levels(TaxTab[,i])
  # refactor to include "None" as a factor level and replace NA with "None"
  levels[length(levels) + 1] <- "None"
  TaxTab[,i] <- factor(TaxTab[,i], levels = levels)
  TaxTab[,i][is.na(TaxTab[,i])] <- "None"
}
# remove species column
TaxTab <- subset(TaxTab, select = -c(Species))
TaxTab <- subset(TaxTab, select = -c(Confidence))
...
ps.REC.glom #  89 taxa and 29 samples 
...
ps.cheese.noL # 88 taxa and 11 samples
...
```{r}
ps.glom.rare <-  rarefy_even_depth(ps.REC.glom, sample.size = 6100, 
                                   replace = TRUE, rngseed = 123 , 
                                   trimOTUs = TRUE, verbose = TRUE)
ps.glom.rare #  171 taxa and 29 samples

# Subset to contain milk samples 
ps.milk.rare <- subset_samples(ps.glom.rare, SampleType %in% c("HTST_feed", "HTST_milk"))
ps.milk.rare #  171 taxa and 18 samples 

# Subset to contain cheese samples ()
ps.cheese.rare <- subset_samples(ps.glom.rare, SampleType %in% "Pilot_cheese_5D")
ps.cheese.rare #  171 taxa and 11 samples 
ps.cheese.rare.noL <- subset_taxa(ps.cheese.rare, !Genus %in% "Lactococcus")
ps.cheese.rare.noL # 166 taxa and 11 samples
