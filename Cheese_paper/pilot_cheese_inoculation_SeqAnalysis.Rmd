...
### For taxonomy and beta (?) analysis based on "ps.Phy.glom"
...
ps.milk <- subset_samples(ps.Phy.glom, SampleType %in% c("HTST_feed", "HTST_milk"))
...
ps.cheese <- subset_samples(ps.Phy.glom, SampleType %in% "Pilot_cheese_5D")
...
#### Bar plot #####
SlitBar <- function(ps, var1, var2, taxa, path.out, w, h, x){  
  ...
  TaxTab2_agg <- aggregate(Abundance ~ TaxTab2[[var1]] + TaxTab2[[var2]] + Genus,
                           data = TaxTab2,
                           mean)
  colnames(TaxTab2_agg)[1] <- var1
  colnames(TaxTab2_agg)[2] <- var2
  TaxTab2_agg$Genus <- as.character(TaxTab2_agg$Genus)
  TaxTab2_agg[TaxTab2_agg$Genus %in% taxa.other,]$Genus <- "Other"
  ...
  pdf(path.out, w, h)
  p <- ggplot(TaxTab2_agg, aes(x = get(var1), y = Abundance, fill = Genus)) +
...
SlitBar(ps.milk, var1 = "SampleID", var2 = "SampleType",
        comm14, path.out = file.path(path, "figs/SampleType_SampleID.pdf"), 6, 4.5)
