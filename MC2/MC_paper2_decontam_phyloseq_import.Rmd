...
#### Taxonomy plot
```{r}
TaxBar <- function(ps, var1, w, h, path.out){
  ...
  TaxTab2_agg <- aggregate(Abundance ~ TaxTab2[[var1]] + REC,
                           data = TaxTab2,
                           mean)
  ...
  (ggplot(TaxTab2_agg, aes(x = get(var1), y = Abundance, fill = REC)) +
    #facet_wrap(~ TaxTab2[[var2]], scales = "free") +
    geom_bar(stat = "identity") + 
    scale_fill_manual(values = mycol)+
    theme_bw(base_size = 15)+
    guides(fill = guide_legend(reverse = FALSE, keywidth = 1, keyheight = 1)) +
    ylab("Relative proportion")) %>% print() 
  dev.off()
}

TaxBar(ps7, var1 = "SampleID", 
       path.out = file.path(path,"feature_table/test.pdf"), w = 9, h = 8)
```
