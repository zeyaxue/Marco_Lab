...
samdf_UHT <- read.csv(file.path(path, "mapping/MC_paper2_mapping_UHT.csv"))
rownames(samdf_UHT) <- samdf_UHT$SampleID
ps8 <- phyloseq(sample_data(samdf_UHT), otu_table(ps.Phy.REC),
                tax_table(ps.Phy.REC), phy_tree(ps.Phy.REC))

# create ps list with all phyloseq objects
psList <- list(ps1, ps2, ps3, ps4, ps5, ps6, ps7, ps8)
names(psList) <- c(1:8)
psList <- lapply(psList, transform_sample_counts, function(x) x/sum(x))
...
# Plot for PMA (ps4)
TaxBar(subset_samples(ps4, SampleID != "Theo2.2" & SampleID != "Theo2.3"), 
       var1 = "SampleID", var2 = "For_AVE",
...
## Use decontam package to see if by removing the UHT taxa, the vol will be better
```{r}
ps.Phy05
```
