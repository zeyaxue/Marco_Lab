...
```{r}
library(questionr)
# Define function of Log odds ratio plot with 95% confidence level (Cl)
LogOR <- function(ps, path.out){
  melt <- psmelt(ps)
  melt$Abundance <- factor(melt$Abundance)
  
  reg <- glm(CheeseOutcome ~ Abundance + Genus, data = melt, family = binomial)
  OR.tab <- odds.ratio(reg, level = 0.95)  # factor level is no slits vs slits
  write.csv(OR.tab, path.out)
  
  OR.tab$LogOR <- log10(OR.tab$OR)
}

LogOR(ps.htst4Y, file.path(path, "milk/fig_LogOR_biomarker/htst4Y.csv"))
