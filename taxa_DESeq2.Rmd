...
TiSinCleaningDA <- function(ps, path.out) {
  psdds <- phyloseq_to_deseq2(ps, ~ TimeSinceCleaning)
  geoMeans <- apply(counts(psdds), 1, gmMean)
  psdds <- estimateSizeFactors(psdds, geoMeans = geoMeans)
  dds <- DESeq(psdds, test = "Wald", fitType = "local") 
  plotDispEsts(dds, ylim = c(1e-6, 1e2)) 
...
ChouHeat <- function(ps, samType, w, h, path.out) {
  ...
  taxa.df <- psmelt(ps)
  taxa.agg <- aggregate(Abundance ~ REC + CheeseOutcome,
                        data = taxa.df,
                        mean)
  taxa.cast <- dcast(taxa.agg, REC ~ CheeseOutcome, mean, value.var = "Abundance")
  #taxa.cast <- t(taxa.cast)
  row.names(taxa.cast) <- taxa.cast[,1]
  ...
  superheat(taxa.cast,
            ...
            order.rows = rev(order(rownames(taxa.cast))),
...
ps.chedY.noL
DA(ps.chedY.noL, var = CheeseOutcome,
...
TaxaHeat(ps.htstY, samType = "PMA treated HTST milk", w = 350, h = 350,
         path.out = file.path(path, "milk/fig/htstY.pdf"))
TiSinCleaningDA(ps.htstY, 
                path.out = file.path(path, "milk/deseq_table_TiSinCleaning/sigtab.htstY.csv"))
...
TiSinCleaningDA(ps.htst4Y, 
                path.out = file.path(path, "milk/deseq_table_TiSinCleaning/sigtab.htst4Y.csv"))
```
