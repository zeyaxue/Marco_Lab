...
# Log odds ratio plot with 95% confidence level (Cl)
```{r}
FisherSlit <- function(ps, Genus){
  melt <- psmelt(ps)
  ...
  agg <- ddply(melt, c("CheeseOutcome","Genus"),
               ...
               sd = sd(Abundance),
               se = sd / sqrt(N))
  Tab <- acast(melt, formula = Genus ~ CheeseOutcome, value.var = "Abundance",
               fun.aggregate = mean)
  fisher.test(Tab, alternative = "greater",workspace=2e9) # Otherwise the workspace runs out of space
                )
