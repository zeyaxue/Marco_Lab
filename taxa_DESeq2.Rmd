...
library(plyr)
library(dplyr)
library(PMCMR) # pairwise comparison
...
# Check for normality of the data before ANOVA
```{r}
NormCheck <- function(ps, x, w, h, path.out){
  melt <- psmelt(ps)
  
  pdf(path.out, width = w, height = h)
  attach(subset(melt, Genus %in% x))
  hist(Abundance, probability = T)
  lines(density(Abundance), col = 2)
  detach()
  dev.off()
}

# plot to get all the normality plots 
for(i in 1:length(tax_table(ps.htstqY)[,6])){
  NormCheck(ps.htstqY, x = tax_table(ps.htstqY)[,6][i], w = 4.5, h = 3.5, 
            path.out = file.path(path, "milk/normality/htstqY", paste(tax_table(ps.htstqY)[,6][i],".pdf", sep ='')))
}
for(i in 1:length(tax_table(ps.feedqY)[,6])){
  NormCheck(ps.feedqY, x = tax_table(ps.feedqY)[,6][i], w = 4.5, h = 3.5, 
            path.out = file.path(path, "milk/normality/feedqY", paste(tax_table(ps.feedqY)[,6][i],".pdf", sep ='')))
}



```
The distribution of taxa was not normal at all....Use Kruskal wallis test instead of ANOVA


## Kruskal Wallis (KW) line figures for each taxa 
```{r}
TiSiCleanKW <- function(ps, x, path.out){
  agg <- psmelt(ps)
  
  fit <- aov(Abundance ~ TimeSinceCleaning, data = subset(agg, Genus %in% x))
  y <- fit[[3]]  # p-value
  # if overall value < 0.05, move on with post hoc test
  if (y == "NaN"){  # if might be NaN value if taxa abudance is 0
    ...
    z <- posthoc.kruskal.dunn.test(Abundance, TimeSinceCleaning, p.adjust.method = "none")[[3]]
    detach()
    write.csv(z, path.out)
  }
}

# Loop KW test for all taxa in HTST milk ## PMA treated
for(i in 1:length(tax_table(ps.htstqY)[,6])){
  TiSiCleanKW(ps.htstqY, x = tax_table(ps.htstqY)[,6][i],
              path.out = file.path(path, "milk/fig_TiSinCleaning/htstqY/KW", paste(tax_table(ps.htstqY)[,6][i],".csv", sep ='')))
}
# Loop ANOVA test for all taxa in HTST milk ## PMA UNtreated
for(i in 1:length(tax_table(ps.htstqN)[,6])){
  TiSiCleanKW(ps.htstqN, x = tax_table(ps.htstqN)[,6][i],
              path.out = file.path(path, "milk/fig_TiSinCleaning/htstqN/KW", paste(tax_table(ps.htstqN)[,6][i],".csv", sep ='')))
}
# Loop ANOVA test for all taxa in HTST feed ## PMA treated
for(i in 1:length(tax_table(ps.feedqY)[,6])){
  TiSiCleanKW(ps.feedqY, x = tax_table(ps.feedqY)[,6][i],
              path.out = file.path(path, "milk/fig_TiSinCleaning/feedqY/KW", paste(tax_table(ps.feedqY)[,6][i],".csv", sep ='')))
}
...
# Make log odds ratios plot for biomarker taxa 
```{r}
library(questionr)
# Define function of Log odds ratio plot with 95% confidence level (Cl) with 
# logistic modelling for more than 2 groups
LogOR <- function(ps, path.out){
  melt <- psmelt(ps)
  agg <- ddply(melt, c("CheeseOutcome", "Genus"),
               summarise,
               sum = sum(Abundance))
  cast <- dcast(agg, Genus ~ CheeseOutcome, value.var = "sum")   # keep in long ~ convert to short format
  
  reg <- glm(cbind(no_slits, slits) ~ Genus, data = cast, family = binomial)
   
  OR.tab <- odds.ratio(reg)  # factor level is no slits vs slits
  OR.tab$LogOR <- log10(OR.tab$OR)
  OR.tab$Log2.5 <- log10(OR.tab[,2])
  OR.tab$Log97.5 <- log10(OR.tab[,3])
  write.csv(OR.tab, path.out)
}

# Define rarefaction function
Rarefy <- function(ps, depth){
  phyloseq::rarefy_even_depth(ps, sample.size = depth, replace = TRUE, 
                              rngseed = 123 , trimOTUs = TRUE, verbose = TRUE)
}

# Define function to plot Log odds ratio plot
LogPlot <- function(df, samType, path.out, w, h){
  ### organize in Excel the output from LogOR()
  pdf(path.out, w, h)
  (ggplot(df, aes(x = X, y = -LogOR, ymin = -Log2.5, ymax = -Log97.5))+  # minus to filp slit and no slit orientation
    geom_point()+
    geom_pointrange() + 
...
# PMA treated HTST milk
LogOR(Rarefy(ps.htst4Y, 2000), 
      file.path(path, "milk/fig_LogOR_biomarker/htst4Y.csv"))
LogPlot(read.csv(file.path(path, "milk/fig_LogOR_biomarker/htst4Y - Copy.csv")), 
        samType = "PMA treated HTST milk",
        path.out = file.path(path, "milk/fig_LogOR_biomarker/htst4Y.pdf"),
        w = 6, h = 4.2)
# PMA treated HTST feed
LogOR(Rarefy(ps.feed4Y, 2000), 
      file.path(path, "milk/fig_LogOR_biomarker/feed4Y.csv"))
LogPlot(read.csv(file.path(path, "milk/fig_LogOR_biomarker/feed4Y copy.csv")), 
        samType = "PMA treated HTST feed",
        path.out = file.path(path, "milk/fig_LogOR_biomarker/feed4Y.pdf"),
        w = 6, h = 4.2)

# PMA treated cheese 0 day
LogOR(ps.chedY.noL0, 
      file.path(path, "cheddar/fig_LogOR_biomarker/chedY.noL0.csv"))
LogPlot(read.csv(file.path(path, "cheddar/fig_LogOR_biomarker/chedY.noL0 copy.csv")), 
        samType = "PMA treated cheddar 0 day",
        path.out = file.path(path, "cheddar/fig_LogOR_biomarker/chedY.noL0.pdf"),
        w = 6, h = 4.2)
# PMA treated cheese 30 day
LogOR(ps.chedY.noL30, 
      file.path(path, "cheddar/fig_LogOR_biomarker/chedY.noL30.csv"))
LogPlot(read.csv(file.path(path, "cheddar/fig_LogOR_biomarker/chedY.noL30 copy.csv")), 
        samType = "PMA treated cheddar 30 day",
        path.out = file.path(path, "cheddar/fig_LogOR_biomarker/chedY.noL30.pdf"),
        w = 6, h = 4.2)
# PMA treated cheese 90 day
LogOR(ps.chedY.noL90, 
      file.path(path, "cheddar/fig_LogOR_biomarker/chedY.noL90.csv"))
LogPlot(read.csv(file.path(path, "cheddar/fig_LogOR_biomarker/chedY.noL90 copy.csv")), 
        samType = "PMA treated cheddar 90 day",
        path.out = file.path(path, "cheddar/fig_LogOR_biomarker/chedY.noL90.pdf"),
        w = 6, h = 4.2)
# PMA treated cheese 120 day
LogOR(ps.chedY.noL120, 
      file.path(path, "cheddar/fig_LogOR_biomarker/chedY.noL120.csv"))
LogPlot(read.csv(file.path(path, "cheddar/fig_LogOR_biomarker/chedY.noL120 copy.csv")), 
        samType = "PMA treated cheddar 120 day",
        path.out = file.path(path, "cheddar/fig_LogOR_biomarker/chedY.noL120.pdf"),
        w = 6, h = 4.2)
...
# Import Lactobacillus qPCR results 
Only PMA samples were quantified using qPCR. 
```{r}
qPCR.Lb <- read.csv(file.path(path, "mapping/Lactobacillus_qPCR_ZX.csv"))

# melt dataframe to be suited for ploting
qPCR.Lbm <- melt(qPCR.Lb, id.vars = c("Week","CheeseOutcome","SampleType","Lb_helveticus","Lb_fermentum"),
                 variable.name = "measure",
                 value.name = "value")
qPCR.Lbm$SampleType <- factor(qPCR.Lbm$SampleType, 
                              levels = c("HTST_feed","HTST_milk", 
                                         "Cheese_0D","Cheese_30D","Cheese_90D",
                                         "Cheese_120D"))

# Define plotting function for helveticus data set 
LbhelPlot <- function(df, path.out, w, h) {
  pdf(file.path("~/Google Drive File Stream/My Drive/UC_Davis/Marco_lab/milk_microbiota/Hilmar_weekly_samples/",path.out), width = w, height = h)
  print(ggplot(df, aes(x=SampleType, y=Lb_helveticus, fill=CheeseOutcome))+ 
    geom_boxplot() +
    scale_fill_manual(values=c("no_slits"="#a6cee3", "slits"="#1f78b4")) +
    theme_bw(base_size = 15)+
    ylab("Log10 cells/ml")+
    ggtitle("Lactobacillus helveticus"))
  dev.off()
}
LbhelPlot(subset(qPCR.Lbm, SampleType %in% c("HTST_feed", "HTST_milk")),
          path.out = "Lactobacillus qPCR/results/Lb_hel_milk.pdf", 6, 3.8)
LbhelPlot(subset(qPCR.Lbm, SampleType %in% c("Cheese_0D", "Cheese_30D", "Cheese_90D", "Cheese_120D")),
          path.out = "Lactobacillus qPCR/results/Lb_hel_cheese.pdf", 7.5, 3.8)

# Define plotting function for fermentum data set 
LbfermPlot <- function(df, path.out, w, h) {
  pdf(file.path("~/Google Drive File Stream/My Drive/UC_Davis/Marco_lab/milk_microbiota/Hilmar_weekly_samples/",path.out), width = w, height = h)
  print(ggplot(df, aes(x=SampleType, y=Lb_fermentum, fill=CheeseOutcome))+ 
    geom_boxplot() +
    scale_fill_manual(values=c("no_slits"="#a6cee3", "slits"="#1f78b4")) +
    theme_bw(base_size = 15)+
    ylab("Log10 cells/ml")+
    ggtitle("Lactobacillus fermentum"))
  dev.off()
}
LbfermPlot(subset(qPCR.Lbm, SampleType %in% c("HTST_feed", "HTST_milk")),
           path.out = "Lactobacillus qPCR/results/Lb_ferm_milk.pdf", 6, 3.8)
LbfermPlot(subset(qPCR.Lbm, SampleType %in% c("Cheese_0D", "Cheese_30D", "Cheese_90D", "Cheese_120D")),
           path.out = "Lactobacillus qPCR/results/Lb_ferm_cheese.pdf", 7.5, 3.8)



# Define Kruskal Wallis test for helveticus data set 
helveticusKW <- function(x, sample){
  kruskal.test(subset(x, SampleType %in% sample), Lb_helveticus ~ CheeseOutcome)
  attach(subset(x, SampleType %in% sample))
  # because Nemenyi is no appropriate for groups with unequal sample sizes
  z <- posthoc.kruskal.dunn.test(Lb_helveticus, CheeseOutcome, p.adjust.method = "none")[[3]]
  detach()
  z
}
helveticusKW(qPCR.Lbm, sample = "HTST_feed")
helveticusKW(qPCR.Lbm, sample = "HTST_milk")
helveticusKW(qPCR.Lbm, sample = "Cheese_0D")
helveticusKW(qPCR.Lbm, sample = "Cheese_30D")
helveticusKW(qPCR.Lbm, sample = "Cheese_90D")
helveticusKW(qPCR.Lbm, sample = "Cheese_120D")


# Define Kruskal Wallis test for fermentum data set 
fermentumKW <- function(x, sample){
  kruskal.test(subset(x, SampleType %in% sample), Lb_fermentum ~ CheeseOutcome)
  attach(subset(x, SampleType %in% sample))
  # because Nemenyi is no appropriate for groups with unequal sample sizes
  z <- posthoc.kruskal.dunn.test(Lb_helveticus, CheeseOutcome, p.adjust.method = "none")[[3]]
  detach()
...
fermentumKW(qPCR.Lbm, sample = "HTST_feed")
fermentumKW(qPCR.Lbm, sample = "HTST_milk")
fermentumKW(qPCR.Lbm, sample = "Cheese_0D")
fermentumKW(qPCR.Lbm, sample = "Cheese_30D")
fermentumKW(qPCR.Lbm, sample = "Cheese_90D")
fermentumKW(qPCR.Lbm, sample = "Cheese_120D")



## Import the qPCR results for Turicibacter counts in cheese and milk
```{r}
qPCR.Tur <- read.csv(file.path(path, "mapping/Good_data_Turicibacter_qPCR-1-6-19.csv"))
qPCR.Tur <- qPCR.Tur[,-3] #remove old log value
# subset by PMA treated and untreated
qPCR.TurY <- qPCR.Tur[qPCR.Tur$PMA == "Y",]
qPCR.TurN <- qPCR.Tur[qPCR.Tur$PMA == "N",] 
# melt dataframe to be suited for ploting
qPCR.TurYm <- melt(qPCR.TurY, id.vars = c("Week","CheeseOutcome","SampleType","LOG2"),
                   variable.name = "measure",
                   value.name = "value")
qPCR.TurNm <- melt(qPCR.TurY, id.vars = c("Week","CheeseOutcome","SampleType","LOG2"),
                   variable.name = "measure",
                   value.name = "value")
...
TuriPlot <- function(df, path.out, w, h){
  pdf(file.path("~/Google Drive File Stream/My Drive/UC_Davis/Marco_lab/milk_microbiota/Hilmar_weekly_samples/",path.out), width = w, height = h)
  print(ggplot(df, aes(x=SampleType, y=LOG2, fill=CheeseOutcome))+ 
          geom_boxplot() +
          scale_fill_manual(values=c("no_slits"="#a6cee3", "slits"="#1f78b4")) +
          theme_bw(base_size = 15)+
          ylab("Log10 cells/ml")+
...
TuriPlot(subset(qPCR.TurYm, SampleType %in% c("HTST_feed", "HTST_milk")),
         path.out = "Turicibacter qPCR/Turi_milkY.pdf", 6, 3.8)
TuriPlot(subset(qPCR.TurNm, SampleType %in% c("HTST_feed", "HTST_milk")),
         path.out = "Turicibacter qPCR/Turi_milkN.pdf", 6, 3.8)
TuriPlot(subset(qPCR.TurYm, SampleType %in% c("Cheese_0D", "Cheese_30D", "Cheese_90D", "Cheese_120D")),
         path.out = "Turicibacter qPCR/Turi_cheeseY.pdf", 7.5, 3.8)
TuriPlot(subset(qPCR.TurNm, SampleType %in% c("Cheese_0D", "Cheese_30D", "Cheese_90D", "Cheese_120D")),
         path.out = "Turicibacter qPCR/Turi_cheeseN.pdf", 7.5, 3.8)
