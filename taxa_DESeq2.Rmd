...
# Milk heat plot 2 with all the taxa and taxa abudance clustering
MilkHeat2 <- function(ps, samType, w, h, path.out) {
  my_palette <- colorRampPalette(c("red", "yellow", "green"))(n = 299)
  # defines the color breaks manually for a "skewed" color transition
  col_breaks = c(seq(-1,0,length=100),      # for red
                 seq(0.01,0.8,length=100),  # for yellow
...
TiSinBar <- function(ps, taxa, samType, w, h, path.out){
  ...
  print(ggplot(TaxTab2_agg, aes(x = TimeSinceCleaning, y = Abundance, fill = Genus)) +
                ...
                guides(fill = guide_legend(reverse = FALSE, keywidth = 1, keyheight = 1)) +
                ylab("Log10 16S rRNA gene copy/ml"))
  dev.off()
}
```
...
CheeseOutcomeDAnoMean(ps.htstY, 
...
saveRDS(ps.htstY, file.path(path, "milk/ps.htstY.rds"))
ps.htst4Y
CheeseOutcomeDAnoMean(ps.htst4Y, 
...
ps.feed4Y
CheeseOutcomeDAnoMean(ps.feed4Y, 
