...
LogPlot <- function(df, samType, path.out, w, h){
  ...
  pdf(path.out, w, h)
  (ggplot(df, aes(x = X, y = LogOR, ymin = Log2.5, ymax = Log97.5))+
    geom_point()+
    geom_pointrange() + 
...
```{r}
# 12/13/2018 !!!!!!!! Need to update once Kay finished all qPCR
...
ggplot(qPCR.Lbm, aes(x=SampleType, y=Lb_helveticus, fill=CheeseOutcome))+ 
  geom_boxplot() +
...
ggplot(qPCR.Lbm, aes(x=SampleType, y=Lb_fermentum, fill=CheeseOutcome))+ 
  geom_boxplot() +
  scale_fill_manual(values=c("no_slits"="#a6cee3", "slits"="#1f78b4")) +
  theme_bw()+
  theme(text = element_text(size = 15),axis.text.x = element_text(angle = 45, hjust = 1))+
